...
class LiveSensorMonitor:
    ...
    def parse_line(self, text):
        ...
        extracted = {}
        patterns = {
            "nitrogen": [r"\bn\s*[:=]\s*(-?\d+(?:\.\d+)?)", r"\bnitrogen\s*[:=]\s*(-?\d+(?:\.\d+)?)"],
            "phosphorus": [r"\bp\s*[:=]\s*(-?\d+(?:\.\d+)?)", r"\bphosphorus\s*[:=]\s*(-?\d+(?:\.\d+)?)"],
            "potassium": [r"\bk\s*[:=]\s*(-?\d+(?:\.\d+)?)", r"\bpotassium\s*[:=]\s*(-?\d+(?:\.\d+)?)"]
        }
    ...
    def commit_partial_values(self):
        if not self.partial_values: return
        with self.lock:
        ...
        while self.running:
            try:
                ...
                if raw:
                    ...
                    if parsed: self.partial_values.update(parsed)
    ...
    def get_latest(self):
        ...
        if payload["values"]["nitrogen"] is None: payload["values"]["nitrogen"] = 520.0
        if payload["values"]["phosphorus"] is None: payload["values"]["phosphorus"] = 10.0
        if payload["values"]["potassium"] is None: payload["values"]["potassium"] = 10.0
        return payload

# ============================================================================
# SHARED SERIAL LINE BROADCASTER
# Opens COM3 ONCE and fans out every received line to all subscriber queues.
# Eliminates the port-conflict when both FertilizerSensorMonitor and
# CostSensorMonitor need lines from the same physical COM3 device.
# ============================================================================
class SerialLineBroadcaster:
    ...
    def connect(self):
        try:
            if self.ser and self.ser.is_open:
                self.ser.close()
            self.ser = serial.Serial(self.port, self.baudrate, timeout=1)
            print(f"[COM3 BROKER] Connected to {self.port} at {self.baudrate} baud")
            return True
        except Exception as e:
            print(f"[COM3 BROKER] Cannot open {self.port}: {e}")
            return False
    ...
    def start(self):
        ...
        self.thread.start()
        print("[COM3 BROKER] Broadcaster thread started")
    ...
    def _run(self):
        while self.running:
            ...
            try:
                ...
                print(f"[COM3 BROKER] Read error: {e}")
                try:
                    if self.ser:
                        self.ser.close()
                except Exception:
...
# ============================================================================
# DEDICATED FERTILIZER SENSOR MONITOR (For COM3: Env Sensors)
# ============================================================================
class FertilizerSensorMonitor:
    ...
    def start(self):
        ...
        self.thread = threading.Thread(target=self.read_loop, daemon=True)
        self.thread.start()
        print("[FERTILIZER SENSOR] Thread started")

    def start_with_queue(self, line_queue):
        ...
        self.thread.start()
        print("[FERTILIZER SENSOR] Queue-based thread started (via COM3 broker)")
    ...
    def _queue_loop(self, q):
        ...
        while True:
            try:
                text = q.get(timeout=5)          # blocks max 5 s
                line = text.lower()
...
class CostSensorMonitor:
    ...
    def start_with_queue(self, line_queue):
        ...
        self.thread  = threading.Thread(
            target=self._queue_loop, args=(line_queue,), daemon=True)
        self.thread.start()
        print("[FARMING COST SENSOR] Queue-based thread started (via COM3 broker)")
...
# ─────────────────────────────────────────────────────────────────
# COM3 SHARED BROADCASTER
# Opens COM3 ONCE. Both FertilizerSensorMonitor AND CostSensorMonitor subscribe
# to this broadcaster. No port-conflict can occur because only one serial.Serial
# object ever opens COM3.
# ─────────────────────────────────────────────────────────────────
com3_broker   = SerialLineBroadcaster(port="COM3", baudrate=115200)
ferti_queue   = com3_broker.subscribe()   # fertilizer sensor gets its own queue
cost_queue    = com3_broker.subscribe()   # farming cost sensor gets its own queue

ferti_sensor  = FertilizerSensorMonitor(port="COM3", baudrate=115200)
cost_sensor   = CostSensorMonitor(port="COM3", baudrate=115200)

# We use a flag and before_request to prevent the Flask reloader’s master process
# from locking the COM port. Only the worker process connects to the sensor.
...
def start_background_sensors():
    ...
    if not sensors_started:
        # Start the shared COM3 broadcaster first — it opens the port once.
        com3_broker.start()
        # Both monitors now read from their private queue (no direct serial access).
        ferti_sensor.start_with_queue(ferti_queue)
        cost_sensor.start_with_queue(cost_queue)
        sensors_started = True
        print("[STARTUP] COM3 broker + Fertilizer + Farming Cost sensor threads started")
...
def api_reconnect_sensor():
    try:
        # Reconnect BOTH monitors to be safe
        if sensor_monitor:
            sensor_monitor.stop()
            time.sleep(0.5)
            sensor_monitor.start()
